...
T MessageQueue<T>::receive()
{
    ...
    _mqCV.wait(uLock,[this]{return !_queue.empty();}) ;
    T msg = std::move(_queue.front());
    _queue.pop_front();
    return msg; // Return Value Optimization in action here
}
...
void MessageQueue<T>::send(T &&msg)
{
    ...
    std::lock_guard<std::mutex> mqLock(_mqMtx);
...
void TrafficLight::waitForGreen()
{
    ...
    while(true) {
        ...
        TrafficLightPhase tmp = _message.receive() ;
       
        if( tmp == TrafficLightPhase::green) {
            break;
        }
    }
}
...
void TrafficLight::cycleThroughPhases()
{
    ...
    while(true) {
        ...
        if( timeSinceLastUpdate >= randTime*1000 ) {    
